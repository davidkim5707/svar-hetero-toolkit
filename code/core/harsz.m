%==========================================================================
% harsz.m -- HARS-Z sampler.
%   Sign + zero + narrative + regime-heteroskedasticity identification.
%   HARS-Z extends the HARS sampler of Kim and Zha (hars.m) to exact zero
%   restrictions on impulse responses. A0 is common across regimes.
%
%   The file runs as a script. The caller defines
%     data.y, data.varnames, data.filtered_data   the sample
...
%   impulse response to zero exactly. The rotation is built column by column
%   from an n-by-n Gaussian matrix X, and each column is projected onto the
%   null space of its zero constraints, as in Algorithm 2 of Arias,
%   Rubio-Ramirez, and Waggoner (2018). Every rotation the chain visits
%   satisfies the zeros by construction. The constraint object is rebuilt
%   whenever the theta-cache is rebuilt, because its rows depend on A0,
%   Lambda, and the reduced-form dynamics. With options.ZeroRestrictions
%   empty, the zero-aware routines delegate to their HARS counterparts and
%   the sampler runs the HARS path.
%
%   WEIGHTS. When zeros restrict two or more shocks, each retained draw
%   carries the volume element v(Q; theta) of the construction.
%     output.draw_weight        raw volume element v
%     output.draw_block         index of the theta block of the draw
%     output.draw_weightA       v normalized within each theta block
...
%
%   N_ess Q-RETENTION. After the structural move, run N_ess local ESS updates
%   of X under the final theta and retain all of them as posterior draws
%   (serially dependent, but valid draws of Q | theta, SR>0). The storage
%   stage loops options.ess_store_rounds = N_ess times, storing each
%   resulting (theta, Q); this amortises the structural move over several
%   cheap Q-draws. A start-of-sweep pre-mixing loop (options.ess_premix_rounds)
%   helps the move land in an admissible region.
%
%   JOINT (A0,Lambda,A+) STRUCTURAL REPAIR. When an MH-accepted completed draw
%   (A0,Lambda,A+) fails SR(theta',Q_current), the repair redraws the whole
%   structural collection jointly. Attempt up to options.theta_redraw_budget
%   = N_theta joint redraws from the current state, each completed with a
%   fresh A+ and accepted on the A+-collapsed MH ratio times the
%   completed-draw indicator; keep the first admissible candidate. On
%   exhaustion the global Q-repair runs under the accepted theta'. Mirrored
%   in burn-in.
%
%   Impact matrix. With S_m = A0^{-1} Lambda_m^{1/2} Q', a joint (A0,Lambda)
%   move changes Lambda_m^{1/2} and the relative weights inside S_m, so
%   admissibility is re-checked after every accepted (A0,Lambda) move. The
%   average-variance (BPSS) normalization of Lambda is a positive scale and
%   does not flip signs.
%
%   GLOBAL Q-REPAIR. The repair uses the first admissible independent draw,
%   up to N_Q = options.ess_repair_budget attempts. Without zeros the draw is
%   Haar on O(n). With zeros the draw comes from the construction above. If
%   the attempts are exhausted the event is a discarded transition and the
%   sampler holds at its current admissible state.
%
%   NARRATIVE OMEGA TOGGLE. options.use_nrr_omega gates the AD-RR omega
%   importance weighting for narrative restrictions. With use_nrr_omega =
%   false there is no omega_hat simulation and narrative restrictions enter
%   only through the SR indicator. use_nrr_omega = true recovers the
...
% Carry the matching so cache / storage / posterior all agree.
...
% Zero restrictions present? (gate for ALL additions) --------
has_zero_restr = isfield(options,'ZeroRestrictions') && ~isempty(options.ZeroRestrictions);
ZeroRestrictions = [];
if has_zero_restr
    ZeroRestrictions = options.ZeroRestrictions;
end
...
if has_sign || has_narr || has_narr_regime || has_zero_restr   % Zero alone can drive the SR machinery
    ...
    % Shocks carrying a zero restriction also count as "accepted" so
    %        their IRFs are stored / scaled in post-processing.
    if has_zero_restr
        zero_shock_ids = [ZeroRestrictions.shock_idx];
        shock_ids = [shock_ids(:); zero_shock_ids(:)];
    end
    ...
    % The cache must store Psi up to the largest zero horizon too, or
    %        build_zero_constraint cannot form the H-step constraint row.
    if has_zero_restr
        zero_max_h = max([ZeroRestrictions.horizon]);   % economic horizon (0 = impact)
        sr_max_horizon = max(sr_max_horizon, zero_max_h + 1);
        fprintf('  [ZERO] %d zero restriction(s); max zero horizon = %d -> cache horizon raised\n', ...
            numel(ZeroRestrictions), zero_max_h);
    end

    % Narrative dominance in 'interval_hd' mode accumulates the
...
% Upstream rejection buffer. The default is tol = 1e-6;
...
fprintf('  global repair mode      : independent-only (Haar off-manifold / construction measure on it)\n');
...
fprintf('  zero restrictions (ARRW): %s\n', ternary(has_zero_restr, sprintf('ON (%d)', numel(ZeroRestrictions)), 'OFF (byte-identical path)'));
...
%   the SR indicator). Default true applies the AD-RR weighting.
%   Affects: use_draw_weights below, the storage-stage omega block
...
% Single flag for ALL downstream weighting gates. Replace every
% `if has_zero_restr` that guards local_wmedian_dim / local_wquantile_dim
% (SECTIONS 11, 13, 14, and the output.wquantile / output.wmedian handles)
% with `if use_draw_weights`:
% The narrative terms enter ONLY when use_nrr_omega is on;
% zeros keep their own (volume-element) weighting requirement regardless.
% All gated sites consume draw_weight_final_all, assembled
% once after SECTION 12 truncation by local_assemble_weights (the single
% implementation of eq. summary_estimator in this file).
use_draw_weights = has_zero_restr || (use_nrr_omega && (has_narr_regime || has_narr));
...
% Zero-constraint builder wrapper: cache -> per-shock linear constraints
%        zc. When has_zero_restr is false this returns a no-zero stub
%        (build_zero_constraint with [] ZeroRestrictions) so every downstream
%        zero-aware call delegates to its original. validate=true on the first
%        build (rank / feasibility), false thereafter (cheap hot-loop calls).
%
%        zero_on_average (default true): build the zero against the STORED
%        average IRF (regime-1 A0inv, lambda = 1) so the zero holds EXACTLY in
%        output.sign_irfs_temp. Lambda drops out, so the constraint is regime-
%        invariant (one restriction covers all regimes) and the admissible set
%        stays wide. Sign restrictions are still screened in every regime.
%        Set options.zero_on_average = false to recover the per-regime zero.
zero_on_average = get_opt(options, 'zero_on_average', true);
bzc = @(cache, validate) build_zero_constraint(ZeroRestrictions, cache, ...
    sign_regime_dependent, ny, validate, zero_on_average);
...
if use_signrestrictions
    ...
    % Build the zero-constraint for this seed cache (validate once).
    %        zc0.has_zero == false when no zeros -> all zero-aware calls below
    %        delegate to the originals.
    zc0 = bzc(cache0, true);
    if zc0.has_zero
        fprintf('  [ZERO] zc built: %d shock(s) restricted, single_column = %d, feasible = %d\n', ...
            numel(zc0.restricted_cols), zc0.single_column, zc0.feasible);
        if ~zc0.feasible
            error('Zero pattern over-restricts (zc0.feasible == false). Fix options.ZeroRestrictions.');
        end
    end
    ...
    [X_current, Q_current, init_success, init_ntried] = ...
        draw_Q_zero_columnwise( ...
            ny, check_fn_init, cache0.A0inv, cache0.lambda, impact_signs, ...
            sign_regime_dependent, zc0, cw_outer, cw_inner);

    % =====================================================================
    %  [4] FALLBACK 1 — Haar at same theta
    % Haar cannot hit a measure-zero manifold; only used when
    %             there are no zeros. With zeros, skip straight to columnwise
    %             retries (the zero-aware columnwise IS the right primitive).
    % =====================================================================
    if ~init_success && ~zc0.has_zero
    ...
    if ~init_success && use_local_pert_fallback
        ...
        for i_pert = 1:local_pert_tries
            ...
            % Rebuild zc for this perturbed cache (no validate in loop)
            zc_pert = bzc(cache_pert, false);
            ...
            [X_current, Q_current, init_success, n_tried_cw] = ...
                draw_Q_zero_columnwise( ...
                    ...
                    impact_signs, sign_regime_dependent, zc_pert, ...
                    cw_outer_local, cw_inner_local);

            iter_time = toc(iter_start);

            if init_success
                x0 = x0_pert; var0 = var_pert; cache0 = cache_pert; zc0 = zc_pert;
    ...
    if ~init_success
        ...
        fb2_start  = tic;       % Progress timing for the retry loop
        fb2_unstab = 0;         % stable-cache misses
        fb2_cwfail = 0;         % cache stable but Q-search failed
        for i_retry = 1:max_theta_retries
            ...
            if ~stable_retry
                fb2_unstab = fb2_unstab + 1;
                % Progress line even when the cache is unstable, so the
                %        retry loop is never silent (draw_Q_zero_columnwise has
                %        no internal prints). Guarded to the zero path: the
                %        no-zero fallback find_admissible_Q prints its own.
                if has_zero_restr && mod(i_retry, report_every) == 0
                    fprintf('    [Retry %4d/%d] unstable:%d | cw-fail:%d | %.1fs\n', ...
                        i_retry, max_theta_retries, fb2_unstab, fb2_cwfail, toc(fb2_start));
                end
                continue
            end
            ...
            if has_zero_restr
                zc_retry = bzc(cache_retry, false);
                [X_current, Q_current, init_success, ~] = ...
                    draw_Q_zero_columnwise( ...
                        ny, check_fn_retry, cache_retry.A0inv, cache_retry.lambda, ...
                        impact_signs, sign_regime_dependent, zc_retry, ...
                        cw_outer, cw_inner);
            else
            ...
            if init_success
                ...
                if has_zero_restr, zc0 = zc_retry; end
                [lh0, likelihood0, A0_prior0, lambda_prior0, ~] = ...
                    eval_posterior(x0, y, lags, lcA0, lcLmd, Tsigbrk, delta_input_init, tvA, options);
                if has_zero_restr
                    fprintf('    Retry %d: success! (unstable:%d cw-fail:%d, %.1fs)\n', ...
                        i_retry, fb2_unstab, fb2_cwfail, toc(fb2_start));
                else
                fprintf('    Retry %d: success!\n', i_retry);
                end
                break
            else
                fb2_cwfail = fb2_cwfail + 1;
                % Progress line on Q-search failure (zero path only;
                %        no-zero find_admissible_Q prints its own).
                if has_zero_restr && mod(i_retry, report_every) == 0
                    fprintf('    [Retry %4d/%d] unstable:%d | cw-fail:%d | %.1fs\n', ...
                        i_retry, max_theta_retries, fb2_unstab, fb2_cwfail, toc(fb2_start));
                end
    ...
    % Zero-residual sanity: confirm the construction satisfied every
    %        zero EXACTLY at the seed (impact + propagated horizons).
    if zc0.has_zero
        zres = local_zero_residual(Q_current, zc0, cache0, sign_regime_dependent, ny, zero_on_average);
        fprintf('  [ZERO][SANITY] max |zero IRF residual| at seed: %.3e (expect < 1e-10)\n', zres);
        assert(zres < 1e-8, 'Zero restriction NOT satisfied at seed — check build_zero_constraint / draw_Q_zero_columnwise');
    end
    ...
    for iburn = 1:nburn
        ...
        for ipremix = 1:premix_m
            [X_current, Q_current, ess_ne] = ...
                ess_draw_Q_zero_columnwise(X_current, check_fn, zc0, ess_max_shrinks_per_col);
            ...
            if sr_pass
                ...
                if any(lcA0(:)) || any(lcLmd(:))
                    for t_try = 1:theta_redraw_budget
                        ...
                        cache0 = cache_th; zc0 = bzc(cache_th, false);
                ...
                if ~repaired
                    ...
                    % draws only (up to N_Q = repair_k). A columnwise Q is not a
                    % draw from restricted Haar (Prop. diagnostics (i)) and would
                    % leave the Q-block conditional target of Definition 1.
                    check_fn_repair = @(Q) check_sr_pass_cached(Q, cache_star, var_star, ...
                        SignRestrictions, NarrativeRestrictions, sr_max_horizon, ...
                        sign_regime_dependent, penalty_offdiagonal_on, ny);
                    if has_zero_restr
                        [X_repair, Q_repair, repair_ok, ~] = ...
                            find_admissible_Q_zero(check_fn_repair, zc_star, ny, repair_k);
                    else
                    [X_repair, Q_repair, repair_ok, ~] = ...
...
fprintf('  stale-Q repair          : JOINT (A0,Lambda,A+) redraw (Prop 1), then independent-only Q-repair\n');
...
if use_signrestrictions
    ...
    for rescue_round = 1:seed_rescue_rounds
        ...
        if stable0
            if has_zero_restr
                [X_rep, Q_rep, rep_ok, ~] = draw_Q_zero_columnwise( ...
                    ny, check_fn_seed, cache0.A0inv, cache0.lambda, impact_signs, ...
                    sign_regime_dependent, zc0, 500, repair_k);
            else
            ...
            if check_fn_pert(Q_current)
                ...
                if has_zero_restr
                    [X_rep, Q_rep, rep_ok, ~] = draw_Q_zero_columnwise( ...
                        ...
                        sign_regime_dependent, zc_pert, 500, repair_k);
                else
                [X_rep, Q_rep, rep_ok, ~] = find_admissible_Q_columnwise( ...
                    ny, check_fn_pert, cache_pert.A0inv, cache_pert.lambda, impact_signs, ...
                    sign_regime_dependent, 500, repair_k);
...
while collected_draws < target_draws
    ...
    if use_signrestrictions
        ...
        for iess = 1:ness_store
            ...
            tBlk = tic;
            [X_current, Q_current, ess_ne] = ...
                ess_draw_Q_zero_columnwise(X_current, check_fn_store, zc0, ess_max_shrinks_per_col);
            prof.t_ess = prof.t_ess + toc(tBlk); prof.n_ess = prof.n_ess + 1;
            total_ess_evals = total_ess_evals + ess_ne;
            ...
            if has_zero_restr
                logw = log_volume_element_zero(Qdraw_stored, zc0, ny);
                draw_weight_all(collected_draws) = exp(logw);
            else
                draw_weight_all(collected_draws) = 1;
            end

            % Theta-block id: every retained (theta, Q^(r))
            % of the SAME Step-3 sweep shares total_parameter_draws. This is
            % the index d in eq. (summary_estimator); the within-block index r
            % is the retention order. Stored so downstream code never has to
            % reconstruct blocks from run-lengths of draw_lh.
            draw_block_all(collected_draws) = total_parameter_draws;
            ...
            if use_nrr_omega && (has_narr || has_narr_regime)   % point and/or window anchors
                ...
                nrr_omega_opts.validate     = (collected_draws == 1);   % Fast-vs-exact gate
...
if use_signrestrictions
    ...
    fprintf('  Global Q-repair successes: %d / %d (independent-only)\n', total_repair_success, total_repair_attempts);
    ...
    if fallback_share_theta >= 10
        fprintf(['  [NOTE] Fallback supplies >=10%% of theta moves. Under [INDEP-ONLY]\n' ...
                 '         every repaired Q is a valid conditional draw, so this is a pure\n' ...
                 '         efficiency signal: consider raising theta_redraw_budget\n' ...
                 '         (N_theta) so more stale events resolve by joint repair.\n']);
    end
    if use_draw_weights   % Combined v(Q;theta) / omega_hat weights
        vW  = draw_weight_all(1:collected_draws);
        blk = draw_block_all(1:collected_draws);
        omW = draw_omega_all(1:collected_draws);
        [w_all, wA_diag] = local_assemble_weights(vW, blk, omW);   % final = wA ./ omega
        ess_share = (sum(w_all)^2) / (sum(w_all.^2) * max(numel(w_all),1));
        ess_share_g = (sum(vW./omW)^2) / (sum((vW./omW).^2) * max(numel(vW),1));   % global (pre-fix) reference
        fprintf('  [W] volume weights v:   min %.3e | median %.3e | max %.3e\n', ...
            min(vW), median(vW), max(vW));
        [~, ~, gid11] = unique(blk);
        blen11 = accumarray(gid11, 1);
        fprintf('  [W] theta blocks: %d (draws per block: min %d | median %d | max %d)\n', ...
            numel(blen11), min(blen11), round(median(blen11)), max(blen11));
        fprintf('  [W] within-theta (Option A) weight ESS share: %.3f (1.0 = uniform; low => few draws dominate)\n', ess_share);
        fprintf('  [W] global-normalization ESS share (reference only, NOT the estimator): %.3f\n', ess_share_g);
        if any(~isfinite(wA_diag)) %#ok<*NASGU>
            fprintf('  [W][WARN] non-finite block-normalized weights detected; check volume-element underflow.\n');
        end
    end
...
if use_signrestrictions
    ...
    draw_block_all          = draw_block_all(1:collected_draws);
    draw_omega_all          = draw_omega_all(1:collected_draws);
    om_diag_all             = om_diag_all(1:collected_draws);
end

% Assemble the estimator weights ONCE; every downstream
% weighted summary (SECTIONS 13/14 and the output.wquantile / wmedian
% handles) consumes draw_weight_final_all and nothing else, so
% eq. (summary_estimator) has a single implementation in this file
% (local_assemble_weights -> local_block_normalize).
%   draw_weightA_all      = v block-normalized within each theta draw,
%                           w_A(d,r) = v_{d,r} / sum_r' v_{d,r'}; each block
%                           carries equal total mass (Corollary
%                           ratio_identity; invariant to per-theta rescaling
%                           of v).
%   draw_weight_final_all = draw_weightA_all ./ draw_omega_all. With
%                           use_nrr_omega = false (paper baseline),
%                           draw_omega_all == 1 and final == wA.
if use_signrestrictions
    [draw_weight_final_all, draw_weightA_all] = ...
        local_assemble_weights(draw_weight_all, draw_block_all, draw_omega_all);
else
    draw_weightA_all      = ones(collected_draws, 1);
    draw_weight_final_all = ones(collected_draws, 1);
end
...
if use_signrestrictions
    ...
    output.draw_block         = draw_block_all;
    output.draw_weightA       = draw_weightA_all;
    output.draw_weight_final  = draw_weight_final_all;
    output.has_zero_restr     = has_zero_restr;
    ...
    if has_zero_restr
        output.ZeroRestrictions = ZeroRestrictions;
    end
    if use_draw_weights   % Combined-weight handles
        w_all = draw_weight_final_all;
        output.weight_ess_share = (sum(w_all)^2) / (sum(w_all.^2) * max(numel(w_all),1));
        % Global-normalization ESS share kept as a REFERENCE diagnostic only
        % (the pre-fix Option-B pooling); it is NOT used by any estimate.
        w_glob = draw_weight_all ./ draw_omega_all;
        output.weight_ess_share_global = (sum(w_glob)^2) / (sum(w_glob.^2) * max(numel(w_glob),1));
        ...
        output.wquantile = @(X, p, dim) local_wquantile_dim(X, draw_weight_final_all, p, dim);
        output.wmedian   = @(X, dim)    local_wmedian_dim(X, draw_weight_final_all, dim);
    end
end
...
output.ess_diagnostics.repair_order          = 'independent_only';   % Haar (no-zero) / construction measure (zero)
output.ess_diagnostics.use_nrr_omega         = use_nrr_omega;
output.ess_diagnostics.has_zero_restr        = has_zero_restr;
...
if use_signrestrictions && nRegimes > 1
    ...
    %        otherwise use the unweighted median and quantiles.
    if use_draw_weights
        w13 = draw_weight_final_all(1:collected_draws);   % eq. (summary_estimator) weights
...
if fevd_enable && use_signrestrictions && nRegimes > 1 && isfield(output, 'sign_irf_regime')
    ...
    if use_draw_weights
        w14 = draw_weight_final_all(1:collected_draws);   % eq. (summary_estimator) weights
    ...
    if ~isempty(QE_regs)
        ...
        if use_draw_weights   % Weighted draw dim (4)
...
%% Within-theta (block) normalization of the volume weights.
%   wA = local_block_normalize(v, blk) implements the weights of
%   eq. (summary_estimator):
%       w_A(d, r) = v_{d,r} / sum_{r'} v_{d,r'} ,
%   where d indexes theta blocks (draws sharing blk) and r the retained
%   rotations within a block. Every block carries the SAME total mass, so the
%   weighted mean sum(wA .* h) / sum(wA) equals the equal-weighted average
%   over theta of the per-theta ratio estimators of Corollary
%   (ratio_identity), and weighted quantiles use the identical weights. By
%   construction wA is invariant to any per-theta constant rescaling of v
%   (the constant cancels in the within-block ratio).
%   Blocks of length 1 receive weight 1 (the single-draw ratio). Nesting: a
%   constant-v run (no zeros, or zeros on a single first-built column, where
%   log_volume_element_zero returns 0) yields uniform wA, so every weighted
%   summary reduces to its unweighted counterpart (Proposition zero_measure
%   part (a)).
%   Guard: a block whose weights do not sum to a positive finite number
%   (volume-element underflow / overflow) falls back to UNIFORM weights
%   within that block, with a console warning -- never silently.
function wA = local_block_normalize(v, blk)
    v   = v(:);
    blk = blk(:);
    if numel(blk) ~= numel(v)
        error('local_block_normalize: block-id vector must match the weight vector.');
    end
    [~, ~, gid] = unique(blk);    % dense group ids (base MATLAB / Octave)
    bsum        = accumarray(gid, v);
    bad      = ~(isfinite(bsum) & bsum > 0);
    if any(bad)
        warning('local_block_normalize:degenerateBlock', ...
            ['%d theta block(s) have a non-positive or non-finite volume-weight ' ...
             'sum (underflow/overflow of the volume element); using uniform ' ...
             'weights within those blocks.'], sum(bad));
    end
    blen = accumarray(gid, 1);
    wA   = v ./ bsum(gid);
    badd = bad(gid);
    wA(badd) = 1 ./ blen(gid(badd));
end

%% Assembly of the final estimator weights.
%   [wFinal, wA] = local_assemble_weights(v, blk, omega) returns
%       wA     = local_block_normalize(v, blk)      (eq. summary_estimator)
% WFinal = wA ./ omega                        (AD-RR extension)
%   The 1/omega factor is an AD-RR importance correction whose ACROSS-theta
%   variation must be preserved, so it is applied AFTER, and outside of, the
%   within-block normalization of v. With use_nrr_omega = false (the paper's
%   baseline; omega == 1 for every draw) wFinal == wA exactly. This function
%   is the ONLY place in this file where the two weight components are
%   combined.
function [wFinal, wA] = local_assemble_weights(v, blk, omega)
    wA     = local_block_normalize(v, blk);
    wFinal = wA ./ omega(:);
end
...
%% Local zero-residual check (seed sanity only; no sampler effect).
%   Returns max_{restriction} | e_{i0}' Psi_H^{(r)} L_r p_{j0} | for the drawn
%   Q (= P'), using the same transmission convention as build_zero_constraint.
%   This re-derives the IRF from the cache directly, so a small residual
%   confirms the null-space construction (draw_Q_zero_columnwise) really did
%   place the restricted columns in the constraint null space.
function zres = local_zero_residual(Qdraw, zc, cache, sign_regime_dependent, ny, zero_on_average)
    if nargin < 6 || isempty(zero_on_average), zero_on_average = true; end
    zres = 0;
    if ~isfield(zc, 'has_zero') || ~zc.has_zero, return; end
    P = Qdraw.';                       % P(:,j) = p_j
    tvA = isfield(cache, 'tvA') && cache.tvA;
    for k = 1:numel(zc.meta)
        i0 = zc.meta(k).variable_idx;
        j0 = zc.meta(k).shock_idx;
        H  = zc.meta(k).horizon;
        r  = zc.meta(k).regime_idx;
        if isempty(r), r = 1; end

        % Average mode: match the stored average IRF (regime-1 A0inv,
        %        lambda = 1), exactly as build_zero_constraint does, so the
        %        residual is measured against output.sign_irfs_temp.
        if zero_on_average
            if tvA, A0inv_r = cache.A0inv(:, :, 1); else, A0inv_r = cache.A0inv; end
            sqrtlam_r = ones(ny, 1);
            r_eff     = 1;
        else
            if tvA, A0inv_r = cache.A0inv(:, :, r); else, A0inv_r = cache.A0inv; end
            sqrtlam_r = sqrt(max(cache.lambda(:, r), 0));
            r_eff     = r;
        end
        ...
        if H == 0
            ...
            if tvA, Psi_r = cache.Psi(:, :, :, r_eff); else, Psi_r = cache.Psi; end
            PsiH = Psi_r(:, :, H + 1);
        end
        ...
        if zero_on_average || sign_regime_dependent == 0
            ...
            Lr   = A0inv_r .* sqrtlam_r.';
            Mmat = PsiH * Lr;
        end
        irf_val = Mmat(i0, :) * P(:, j0);
        zres = max(zres, abs(irf_val));
    end
end